...
#
# We are not sure if this is still needed in LIBERTY ubuntu packages, but just in case....
#
...
"qpid")
	...
	# crudini --set /etc/neutron/neutron.conf oslo_messaging_qpid qpid_topology_version 2
	;;
...
crudini --set /etc/neutron/neutron.conf keystone_authtoken project_domain_id $keystonedomain
crudini --set /etc/neutron/neutron.conf keystone_authtoken user_domain_id $keystonedomain
...
nova_admin_tenant_id=`openstack project list|grep $keystoneservicestenant|awk '{print $2}'`
...
crudini --set /etc/neutron/neutron.conf nova project_domain_id $keystonedomain
crudini --set /etc/neutron/neutron.conf nova user_domain_id $keystonedomain
crudini --set /etc/neutron/neutron.conf nova region_name $endpointsregion
